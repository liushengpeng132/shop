...
class UserToken extends Token
{
    ...
    protected $account;
    protected $password;
    protected $public_openid;
    protected $wxLoginUrl;
    protected $wxAppID;
    protected $wxAppSecret;

    function __construct($code,$account,$password,$public_openid)
    {
        $this->account = $account;
        $this->password = $password;
        $this->public_openid = $public_openid;
        $this->code = $code;
    ...
    private function grantToken($wxResult)
    {
        ...
        $user = Staff::getByOpenID($openid);
        if (!$user){
            // 借助微信的openid作为用户标识
            // 但在系统中的相关查询还是使用自己的uid
            $uid = $this->newUser($openid,$this->account,$this->password,$this->public_openid);
        }
        else {
            $uid = $user->id;
        }
    ...
    private function prepareCachedValue($wxResult, $uid)
    {
        ...
        $cachedValue['uid'] = $uid;
        $cachedValue['scope'] = Staff::where('id',$uid)->value('wx_scope');
        // $cachedValue['scope'] = ScopeEnum::User;
        return $cachedValue;
    }

    // 创建新用户
    private function newUser($openid,$account,$password,$public_openid)
    {
        // 有可能会有异常，如果没有特别处理
        // 这里不需要try——catch
        // 全局异常处理会记录日志
        ...
        $where['openid'] = ['eq',''];//这个一定得要,否则其他员工就能登录账号密码登录并修改别人的openid
        $user = Staff::where($where)
            ->update([
                'openid' => $openid,
                'public_openid'=>$public_openid
            ]);
        if($user){
           $id =  Staff::where('openid',$openid)->value('id');
        }else{
            exit(json_encode([
                'pass'=>-1,
                'msg'=>'请联系人事部门添加您的账号'
