...
class Milk extends BaseController
{
    ...
    public function getMilkByStoreId()
    {
        ...
        if($search == 'undefined'){
	        ...
	        $res = model('MilkCount')->where($data)->value('id');
	        //如果不存在就初始化加载后再获取
	        if(!$res){
	        	$goodsArr = [];
	        	$i = 0;
        ...
        $milk_counts = model('MilkCount')->where($data)->column('id,goods_id,milk_count');
        // print_r($milk_counts);exit;
        $resArr = [];
        $i = 0;
        foreach ($goods as $k => $v) {
        	foreach ($milk_counts as $k1 => $v1) {
        		if($k == $v1['goods_id']){
        			$resArr[$i]['id'] = $v1['id'];
        			$resArr[$i]['goods_id'] = $v1['goods_id'];
        			$resArr[$i]['milk_count'] = $v1['milk_count'];
        			$resArr[$i]['goods_name'] = $v['goods_name'];
        		}
        	}
        	$i++;
        }
        // echo model('MilkCount')->getLastSql();
        miniapp_log($this->staff_id,'初始化饮料表');
        return $resArr;
    }

	public function getMilkSaleByStoreId()
    {
        $data['store_id'] = input('get.store_id');
        $data['time'] = strtotime(input('get.date'));
        $data['milk_count'] = ['gt',0];
        $search = input('get.search');
 		$goods_where['store_id'] = $data['store_id'];
	    $goods_where['goods_class'] = 0;//4代表库存商品
	    $goods_where['cat_id'] = 896;
    ...
    //写入盘点库存
    public function editMilk()
    {
    	
        $post =  input('post.products/a');
        $arr['id'] = $post['id'];
        $data['milk_count'] = $post['value'];
        $data['staff_id'] = $this->staff_id;//写入员工编号,以最后一个为准
        $res = model('MilkCount')->save($data,$arr);
        if($res){
        	$return['status'] = 1;
        	$return['msg'] = '成功';
        }else{
    ...
    public function getMonthByStoreId(){
    	...
    	$times = model('MilkCount')->distinct(true)->where($where)->column('time');
    	$month = [];
    	foreach ($times as $v) {
    		$month[] = date('Y-m',$v);
    	}
    ...
    public function getAllStockByMonth(){
    	...
    	$times = model('MilkCount')->distinct(true)->where($where)->order('time desc')->column('time');//
    	 // model('MilkCount')->getLastSql();
    	// 获取到当月盘点的次数对应的时间
    	$timesArr = [];
    	...
    	$stock_counts = model('MilkCount')->where($where)->column('id,goods_id,stock_count,time');
    	$stocksArr = []; 
    ...
    public function getStockCats(){
    	$data['store_id'] = input('get.store_id');
    	$data['time'] = strtotime(input('get.date'));
    	$cat_ids = model('MilkCount')->distinct(true)->where($data)->column('cat_id');
    	$where['id'] = ['in',$cat_ids];
    ...
    public function getAllCatsByDate(){
    	...
    	$where['time'] = [['egt',$firstDay],['elt',$lastDay]];
    	$where['store_id'] = $store_id;
    	$cat_ids = model('MilkCount')->distinct(true)->where($where)->column('cat_id');
